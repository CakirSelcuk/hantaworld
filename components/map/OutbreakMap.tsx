...
export default function OutbreakMap({ outbreaks, height = '100%' }: Props) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Clear any stale Leaflet state on the container (hot-reload safety)
      if ((mapRef.current as any)._leaflet_id) {
        (mapRef.current as any)._leaflet_id = undefined;
      }

      map = L.map(mapRef.current!, {
        center: [15, 0], zoom: 2, minZoom: 1, maxZoom: 10,
      ...
      outbreaks.forEach((ob) => {
        const color  = getSeverityColor(ob.status);
        const radius = Math.max(18, Math.min(55, Math.sqrt(ob.confirmedCases) * 1.2));

        const circle = L.circleMarker([ob.coordinates.lat, ob.coordinates.lng], {
          radius, fillColor: color, fillOpacity: 0.22,
          color, weight: 1.5, opacity: 0.8,
          className: ob.severityLevel === 'critical' ? 'critical-marker-pulse' : 'standard-marker',
        }).addTo(map);

        L.circleMarker([ob.coordinates.lat, ob.coordinates.lng], {
          radius: 5, fillColor: color, fillOpacity: 1,
          color: '#fff', weight: 1, opacity: 0.9,
        }).addTo(map);

        circle.on('click',     () => setSelected(ob));
        circle.on('mouseover', function(this: any) { this.setStyle({ fillOpacity: 0.5, weight: 3 }); });
        circle.on('mouseout',  function(this: any) { this.setStyle({ fillOpacity: 0.22, weight: 1.5 }); });
      });
    ...
    <div style={{ position: 'relative', height, width: '100%', background: '#070b14', borderRadius: 'inherit' }}>
      ...
      {loaded && (
        <div style={{ position:'absolute', bottom:40, left:12, zIndex:1000, background:'rgba(7,11,20,0.92)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'10px', padding:'0.75rem 1rem', backdropFilter:'blur(12px)' }}>
          ...
          {(['confirmed','suspected','monitoring','resolved'] as const).map((s) => (
            <div key={s} style={{ display:'flex', alignItems:'center', gap:'0.5rem', marginBottom:'0.3rem' }}>
              <div style={{ width:8, height:8, borderRadius:'50%', background:getSeverityColor(s), flexShrink:0 }} />
              <span style={{ fontFamily:'var(--font-mono)', fontSize:'0.62rem', color:'var(--text-secondary)', textTransform:'capitalize' }}>{s}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {selected && (
        <div style={{ position:'absolute', top:12, right:12, zIndex:1000, width:280, background:'rgba(13,20,38,0.97)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'12px', padding:'1.25rem', backdropFilter:'blur(20px)' }}>
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'0.85rem' }}>
            ...
            <button onClick={() => setSelected(null)} style={{ background:'none', border:'none', color:'var(--text-muted)', cursor:'pointer', fontSize:'1.2rem', lineHeight:1, padding:'0' }}>×</button>
          </div>
