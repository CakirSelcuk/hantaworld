...
import { TrendingUp, TrendingDown, Globe, AlertTriangle, Activity, Users } from 'lucide-react';
...
const STATS = [
  { key: 'totalConfirmedCases', label: 'Confirmed Cases', icon: Activity,      color: '#ef4444', glow: 'rgba(239,68,68,0.15)' },
      ...
      <div className="container-main">
        ...
        <div style={{ marginBottom: '2.5rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem', maxWidth: 560, lineHeight: 1.7, marginBottom: '1.5rem' }}>
            Real-time surveillance data aggregated from WHO, CDC, ECDC, ProMED, and official health ministry reports.
            All data is verified, timestamped, and source-attributed.
          </p>
        ...
        <div className="stats-grid" style={{ background: 'var(--border-subtle)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', border: '1px solid var(--border-glass)', boxShadow: '0 20px 40px rgba(0,0,0,0.4)' }}>
          {STATS.map(({ key, label, icon: Icon, color, glow }) => {
            ...
            return (
              <div key={key} style={{ background: 'var(--bg-card)', padding: '1.75rem', position: 'relative', overflow: 'hidden', transition: 'background 0.2s ease' }}
                className="stat-cell">
                {/* Glow bg */}
