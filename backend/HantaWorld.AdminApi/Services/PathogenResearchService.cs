...
{
    public static PathogenResearchOutputViewModel BuildOutput(Pathogen pathogen, PathogenResearchViewModel model)
    {
        ...
        var sourceUrl = primarySource?.Url ?? "-";
        var reportTitle = $"{pathogen.DisplayName} kaynak kontrol taslağı";
        var reportSummary = "Güncel bir gelişme bulunamadı. Resmi kaynaklar kontrol edildikten sonra rapor metni elle tamamlanmalıdır.";
        var reportContent = string.Join(Environment.NewLine + Environment.NewLine, new[]
        {
            $"{pathogen.DisplayName} için resmi halk sağlığı kaynakları kontrol edilmelidir.",
            "Bu taslak canlı veri yayınlamaz ve otomatik olarak istatistik güncellemez. WHO, CDC, ECDC, Africa CDC, PAHO veya ilgili ulusal sağlık bakanlığı kaynaklarından doğrulama yapıldıktan sonra içerik elle tamamlanmalıdır.",
            "Güncel ve doğrulanmış numerik veri bulunamadıysa vaka, ölüm, etkilenen ülke ve aktif salgın alanlarına değer yazılmamalıdır. 0 değeri yalnızca resmi kaynak açıkça 0 bildirdiyse kullanılmalıdır.",
            $"Kontrol kapsamı: {dateScope}"
        });
        var verificationNote = "Bu rapor kaynaklı virüs taraması ekranında taslak olarak üretildi. Public site İngilizce olduğu için yayınlamadan önce kaynaklar kontrol edilmeli, metin editlenmeli ve gerekli ise İngilizceye çevrilmelidir.";
        ...
        return new PathogenResearchOutputViewModel
        {
            ...
            SourceUrl = sourceUrl == "-" ? string.Empty : sourceUrl,
            PublicationDate = today,
            VerificationNote = verificationNote,
            StatisticsText = statisticsText,
            ReportText = reportText,
            SourcesText = sourcesText,
            AdminNote = "Bu çıktı otomatik yayınlanmadı. Sayılar otomatik kaydedilmedi. Kaynakları kontrol ettikten sonra Salgın İstatistikleri ekranına elle gir veya raporu taslak olarak Salgın Raporları içine ekle.",
            Sources = sources
        };
    }
