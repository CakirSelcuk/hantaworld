...
const STATUS_COLORS: Record<string, string> = {
  confirmed: '#ef4444', suspected: '#f97316', monitoring: '#eab308', resolved: '#22c55e',
};

export default async function MapPage() {
  const outbreaks = await getOutbreaks();
  const active = outbreaks.filter(o => o.status !== 'resolved');
      ...
      <div style={{ paddingTop: 64, height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-primary)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '0.75rem 1.5rem', background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border-subtle)', flexShrink: 0, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--text-muted)' }}>
            {active.length} active zones · {outbreaks.reduce((a, o) => a + o.confirmedCases, 0).toLocaleString()} total confirmed cases
          </span>
          <div style={{ display: 'flex', gap: '0.5rem', marginLeft: 'auto', flexWrap: 'wrap' }}>
            {Object.entries(STATUS_COLORS).map(([s, c]) => (
              <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                <div style={{ width: 7, height: 7, borderRadius: '50%', background: c }} />
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--text-muted)', textTransform: 'capitalize' }}>{s}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ width: 300, background: 'var(--bg-secondary)', borderRight: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {outbreaks.map((ob) => (
                <div key={ob.id} style={{ padding: '0.85rem 1rem', borderBottom: '1px solid var(--border-subtle)', transition: 'background 0.15s' }}
                  className="sidebar-item">
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.4rem' }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: getSeverityColor(ob.status), flexShrink: 0 }} />
                    <span style={{ fontSize: '1rem' }}>{ob.country.flagEmoji}</span>
                    <span style={{ fontFamily: 'var(--font-ui)', fontWeight: 600, fontSize: '0.8rem', color: 'var(--text-primary)', flex: 1 }}>{ob.country.name}</span>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.58rem', color: getSeverityColor(ob.status), textTransform: 'uppercase', padding: '0.1rem 0.35rem', borderRadius: '3px', background: `${getSeverityColor(ob.status)}15` }}>
                      {ob.status}
                    </span>
                  </div>
                  <div style={{ display: 'flex', gap: '1rem', paddingLeft: '1.25rem' }}>
                    <div>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.85rem', color: 'var(--text-primary)' }}>{formatNumber(ob.confirmedCases)}</div>
                    ...
                    <div>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.85rem', color: '#f87171' }}>{formatNumber(ob.deaths)}</div>
                  ...
                  <div style={{ marginTop: '0.35rem', paddingLeft: '1.25rem', fontFamily: 'var(--font-mono)', fontSize: '0.58rem', color: 'var(--text-muted)' }}>
                    Updated {timeAgo(ob.lastUpdated)}
                  </div>
                </div>
              ))}
            </div>
